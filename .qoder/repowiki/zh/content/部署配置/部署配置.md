...
- [scripts/smoke.sh](file://scripts/smoke.sh)
- [scripts/seed-data.mjs](file://scripts/seed-data.mjs)
...
## 更新摘要
**所做更改**
- 新增构建自动化与开发工作流章节，详细介绍 Makefile 和脚本工具
- 更新开发环境启动流程，包含一键启动和健康检查机制
- 新增压力测试和数据种子生成工具的使用指南
- 补充本地开发与生产部署的最佳实践对比
- 更新部署工具链的完整生命周期管理
...
**更新** 新增构建自动化与开发工具链章节，详细介绍 Makefile 和脚本工具的使用方法。
...
MK["Makefile"]
DEV["开发脚本<br/>dev.sh"]
SMOKE["压力测试<br/>smoke.sh"]
SEED["数据种子<br/>seed-data.mjs"]
end
DC --> PSQL
DC --> REDIS
DC --> RP
...
- [scripts/dev.sh:1-79](file://scripts/dev.sh#L1-L79)
- [scripts/smoke.sh:1-13](file://scripts/smoke.sh#L1-L13)
- [scripts/seed-data.mjs:1-263](file://scripts/seed-data.mjs#L1-L263)
...
### Makefile 构建任务
AeroLog 提供了完整的 Makefile 构建系统，支持开发、测试、部署等全生命周期操作：

- **开发环境启动**：`make dev` - 一键启动完整开发环境
- **基础设施管理**：`make infra` - 启动/停止 Docker Compose 服务
- **服务启动**：`make api`、`make collector`、`make consumer` - 各服务独立运行
- **前端开发**：`make web` - 启动 Next.js 开发服务器
- **测试执行**：`make test` - 执行所有模块的单元测试
- **压力测试**：`make smoke` - 执行基础健康检查
- **数据种子**：`make seed` - 生成演示数据

**章节来源**
- [Makefile:1-35](file://Makefile#L1-L35)

### 开发环境自动化脚本
`scripts/dev.sh` 提供了智能的开发环境启动能力：

- **进程管理**：自动启动并管理多个服务进程，包含 PID 文件管理和优雅关闭
- **健康检查**：自动检测服务就绪状态，支持超时重试机制
- **日志管理**：集中化的日志输出到 `.run/logs` 目录
- **环境隔离**：支持跳过基础设施启动，专注于特定服务开发

**章节来源**
- [scripts/dev.sh:1-79](file://scripts/dev.sh#L1-L79)

### 压力测试工具
`scripts/smoke.sh` 提供基础的健康检查能力：

- **API 健康检查**：验证 `/healthz` 端点可用性
- **功能验证**：检查项目列表接口的基本功能
- **环境适配**：支持自定义 API 基础地址
...
### 数据种子生成工具
`scripts/seed-data.mjs` 是强大的数据生成工具：

- **用户模拟**：生成指定数量的模拟用户数据
- **行为建模**：模拟真实用户的行为序列和购买流程
- **配置灵活**：支持通过环境变量调整用户数量、时间范围、批次大小等参数
- **批量发送**：支持分批发送事件到收集器，模拟真实流量
...
- [server/api/cmd/main.go:1-26](file://server/api/cmd/main.go#L1-L26)
...
## 结论
通过 Docker Compose 快速搭建 AeroLog 基础设施，结合 Prometheus 与 Grafana 实现可观测性闭环；配合完善的初始化脚本与指标体系，可在本地与生产环境稳定运行。新增的构建自动化工具链进一步提升了开发效率，包括一键启动、健康检查、压力测试和数据生成等功能。建议在生产中强化密钥管理、启用鉴权与 TLS、按业务峰值规划资源并建立自动化备份与告警流程。
...
### 构建命令参考
- **开发环境**：`make dev` - 启动完整开发环境
- **基础设施**：`make infra` - 管理 Docker Compose 服务
- **服务调试**：`make api`、`make collector`、`make consumer` - 各服务独立运行
- **前端开发**：`make web` - 启动 Next.js 开发服务器
- **测试执行**：`make test` - 执行全栈测试
- **健康检查**：`make smoke` - 基础健康验证
- **数据生成**：`make seed` - 生成演示数据
