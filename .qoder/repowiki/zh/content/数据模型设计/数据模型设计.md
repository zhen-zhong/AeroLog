...
### 新增：ClickHouse 用户身份表
**更新** 新增ClickHouse用户身份表，专门用于存储用户身份相关信息，支持高吞吐的身份查询与关联分析。

- 设计思路
  - 用户身份表独立于事件宽表，专注于用户身份维度数据的快速查询。
  - 采用列式存储优化身份相关查询的性能，支持大规模用户身份数据的实时分析。
- 字段设计要点
  - 用户标识：用户ID、外部ID、匿名ID等身份标识符。
  - 身份属性：用户类型、注册渠道、设备指纹、IP地址等身份特征。
  - 时间维度：首次注册时间、最后活跃时间、身份更新时间等时间戳。
- 数据类型与约束
  - 采用低基数字符串类型存储身份标识，提高压缩比。
  - 时间戳字段支持高精度时间类型，确保时间序列分析准确性。
  - 唯一索引确保用户身份数据的完整性与去重。
...
**图表来源**
- [20260617_user_identity.sql:1-200](file://deploy/migrations/clickhouse/20260617_user_identity.sql#L1-L200)

**章节来源**
- [20260617_user_identity.sql:1-200](file://deploy/migrations/clickhouse/20260617_user_identity.sql#L1-L200)

### 新增：PostgreSQL 治理元数据表
**更新** 新增PostgreSQL治理元数据表，用于存储平台治理相关的元数据信息，支持合规性管理和审计追踪。

- 设计思路
  - 治理元数据表用于存储平台级别的治理信息，包括合规设置、审计日志、数据分类等。
  - 与传统的用户、项目表分离，确保治理数据的独立性和安全性。
- 字段设计要点
  - 治理配置：合规政策、数据保留策略、访问控制规则等配置信息。
  - 审计追踪：操作日志、变更历史、访问记录等审计数据。
  - 数据分类：敏感数据标识、数据分级、隐私保护级别等分类信息。
- 数据类型与约束
  - JSONB字段存储灵活的治理配置和审计数据结构。
  - 时间戳字段确保审计数据的时间顺序和可追溯性。
  - 外键约束确保治理数据与业务实体的关联完整性。
...
**图表来源**
- [20260617_governance_metadata.sql:1-200](file://deploy/migrations/postgres/20260617_governance_metadata.sql#L1-L200)
...
PROJECT ||--o{ MEMBER : "拥有"
USER ||--o{ MEMBER : "加入"
USER_IDENTITY ||--o{ EVENT : "关联"
GOVERNANCE_METADATA ||--|| PROJECT : "治理"
GOVERNANCE_METADATA ||--|| USER : "治理"
```
