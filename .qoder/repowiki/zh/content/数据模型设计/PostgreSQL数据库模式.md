...
## 更新摘要
**变更内容**
- 事件定义表新增schema_required_props JSONB列和schema_locked布尔标志
- 属性定义表新增schema_required和schema_locked字段
- 优化调试事件表结构，增强模式锁定功能
- 更新数据治理元数据迁移脚本
...
- **模式控制**：通过schema_locked标志实现事件模式锁定
...
M[20260617_governance_metadata.sql<br/>治理元数据迁移]
end
subgraph "应用层"
A[API服务<br/>Go语言]
W[Web前端<br/>Next.js]
...
- [20260617_governance_metadata.sql:1-50](file://deploy/migrations/postgres/20260617_governance_metadata.sql#L1-L50)
...
存储事件的元数据定义，包括事件名称、显示名称、状态信息和模式控制。

**已更新**：新增schema_required_props JSONB列和schema_locked布尔标志，支持事件模式锁定和必需属性管理。
...
| schema_required_props | JSONB | NOT NULL, DEFAULT '[]'::jsonb | 必需属性列表（JSONB格式） |
| schema_locked | BOOLEAN | NOT NULL, DEFAULT false | 模式锁定标志，true表示禁止修改 |
...
存储属性的元数据定义，支持事件级和用户级属性。**已更新**：扩展了模式控制功能。
...
| schema_required | BOOLEAN | NOT NULL, DEFAULT false | 是否为必需属性 |
| schema_locked | BOOLEAN | NOT NULL, DEFAULT false | 模式锁定标志，true表示禁止修改 |
| enum_values | JSONB | NOT NULL, DEFAULT '[]'::jsonb | 枚举值列表（JSONB格式） |
...
- **新增**：schema_required_props和enum_values字段设置默认值确保JSONB数据完整性

#### 检查约束
- 状态字段使用SMALLINT类型，配合业务逻辑确保有效值范围
- 角色字段使用枚举值，限制可能的取值
- 数据类型字段使用枚举值，限制可能的取值
- **新增**：schema_locked和schema_required字段使用布尔值，确保模式控制的正确性
...
Identify --> Events["事件定义查询<br/>- 项目事件列表<br/>- 事件状态管理<br/>- 事件模式锁定查询"]
Identify --> Properties["属性定义查询<br/>- 项目属性列表<br/>- 属性状态管理<br/>- 属性时间范围查询<br/>- 属性模式锁定查询"]
...
#### JSONB字段
- 使用JSONB存储灵活的配置数据，**已更新**：schema_required_props和enum_values字段
- 支持高效的查询和索引操作
- 默认值使用'[]'::jsonb，确保空数组的正确表示

#### 布尔字段
- 使用BOOLEAN类型存储schema_locked和schema_required标志
- 默认值确保新记录的正确状态
- 支持高效的布尔查询和过滤
...
**新增功能**：通过schema_locked标志实现事件和属性模式的锁定控制。

```mermaid
flowchart TD
LockMechanism[模式锁定机制] --> EventLock[事件模式锁定]
LockMechanism --> PropertyLock[属性模式锁定]
EventLock --> CheckEvent[检查事件schema_locked标志]
PropertyLock --> CheckProperty[检查属性schema_locked标志]
CheckEvent --> IsLocked{schema_locked = true?}
CheckProperty --> IsLocked2{schema_locked = true?}
IsLocked --> Yes1[拒绝修改请求]
IsLocked2 --> Yes2[拒绝修改请求]
IsLocked --> No1[允许修改请求]
IsLocked2 --> No2[允许修改请求]
Yes1 --> End([操作被拒绝])
Yes2 --> End
No1 --> End
No2 --> End
...
**图表来源**
- [project.go:206-212](file://server/api/internal/handler/project.go#L206-L212)
...
4. **事件定义表**：为`project_id`、`name`、`status`和`schema_locked`字段建立索引
5. **属性定义表**：为`project_id`、`name`、`status`、`first_seen`、`last_seen`和`schema_locked`字段建立索引
...
5. **模式锁定查询**：利用schema_locked字段快速筛选锁定状态
...
- 合理设计表结构，避免冗余字段
- **新增**：JSONB字段使用默认值减少NULL值存储

#### 归档策略
- 定期清理历史数据，保持表大小可控
- 对不常访问的数据进行归档
- **新增**：模式锁定的数据应谨慎归档，避免影响现有查询

### 模式锁定性能影响

**新增考虑**：模式锁定功能对性能的影响评估

1. **查询性能**：schema_locked字段的布尔索引应提供快速筛选能力
2. **写入性能**：模式锁定的更新操作应尽量批量处理
3. **存储开销**：布尔字段占用极小存储空间，影响可忽略
4. **索引维护**：需要为schema_locked字段维护额外索引
...
3. **模式锁定功能问题**
   - **新增**：检查schema_locked字段是否正确设置
   - **新增**：验证模式锁定的业务逻辑是否正确执行
   - **新增**：确认JSONB字段的数据格式是否正确
...
6. **模式控制**：通过schema_locked标志实现事件和属性模式的锁定控制
7. **灵活存储**：JSONB字段支持灵活的数据结构存储
...
6. **模式管理**：建立完善的模式版本控制和变更管理流程
7. **JSONB优化**：针对JSONB查询模式优化索引和查询语句

该数据库模式为AeroLog平台提供了坚实的数据基础，支持平台的持续发展和业务扩展需求。新增的属性状态管理、身份映射功能以及模式锁定机制进一步增强了系统的数据治理能力和用户体验追踪能力，为构建企业级埋点平台奠定了坚实的数据库基础。