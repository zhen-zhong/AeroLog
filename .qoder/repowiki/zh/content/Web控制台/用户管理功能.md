# 用户管理功能

<cite>
**本文档引用的文件**
- [users-page.tsx](file://web/src/features/users/users-page.tsx)
- [page.tsx](file://web/src/app/admin/users/page.tsx)
- [page.tsx](file://web/src/app/console/users/page.tsx)
- [governance.go](file://server/api/internal/handler/governance.go)
- [analytics.go](file://server/api/internal/handler/analytics.go)
- [api.ts](file://web/src/lib/api.ts)
- [dashboard-shell.tsx](file://web/src/components/layout/dashboard-shell.tsx)
- [app.go](file://server/api/internal/app/app.go)
- [20260617_governance_metadata.sql](file://deploy/migrations/postgres/20260617_governance_metadata.sql)
</cite>

## 更新摘要
**所做更改**
- 新增用户事件时间线功能章节，详细介绍用户行为轨迹追踪
- 更新核心组件分析，增加用户事件时间线组件说明
- 新增用户事件查询流程图和时间线展示架构图
- 更新 API 接口文档，添加用户事件查询接口
- 增强性能考虑部分，包含时间线查询优化策略

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构概览](#架构概览)
5. [详细组件分析](#详细组件分析)
6. [新增功能：用户事件时间线](#新增功能用户事件时间线)
7. [依赖关系分析](#依赖关系分析)
8. [性能考虑](#性能考虑)
9. [故障排除指南](#故障排除指南)
10. [结论](#结论)

## 简介

AeroLog 是一个实时事件分析平台，用户管理功能是其核心模块之一。该功能允许用户查看和管理项目中的用户信息、身份映射以及用户属性配置。系统采用前后端分离架构，前端使用 Next.js 构建用户界面，后端使用 Go 语言开发 API 服务。

**重要更新**：新增用户事件时间线功能，允许分析师查看个体用户的行为轨迹，包括事件发生的时间顺序、事件类型和属性信息。

用户管理功能主要包含以下核心能力：
- 用户列表展示和搜索
- 用户身份映射查询
- 用户属性配置管理
- 用户画像分析
- **用户事件时间线追踪**（新增）
- 多项目用户管理

## 项目结构

AeroLog 项目采用模块化设计，用户管理功能分布在多个层次中：

```mermaid
graph TB
subgraph "前端层"
WebUI[Web 应用]
AdminUI[管理界面]
ConsoleUI[控制台界面]
UsersPage[用户页面组件]
EventTimeline[事件时间线组件]
end
subgraph "后端层"
API[API 服务]
Handler[治理处理器]
AnalyticsHandler[分析处理器]
Service[业务服务]
end
subgraph "数据层"
Postgres[PostgreSQL]
ClickHouse[ClickHouse]
Migration[数据库迁移]
end
WebUI --> API
AdminUI --> API
ConsoleUI --> API
UsersPage --> EventTimeline
API --> Handler
API --> AnalyticsHandler
Handler --> Service
AnalyticsHandler --> Service
Service --> Postgres
Service --> ClickHouse
Migration --> Postgres
```

**图表来源**
- [users-page.tsx:1-30](file://web/src/features/users/users-page.tsx#L1-L30)
- [governance.go:1-25](file://server/api/internal/handler/governance.go#L1-L25)
- [analytics.go:219-283](file://server/api/internal/handler/analytics.go#L219-L283)
- [app.go:113-119](file://server/api/internal/app/app.go#L113-L119)

**章节来源**
- [users-page.tsx:1-30](file://web/src/features/users/users-page.tsx#L1-L30)
- [page.tsx:1-3](file://web/src/app/admin/users/page.tsx#L1-L3)
- [page.tsx:1-3](file://web/src/app/console/users/page.tsx#L1-L3)
- [governance.go:1-25](file://server/api/internal/handler/governance.go#L1-L25)
- [analytics.go:219-283](file://server/api/internal/handler/analytics.go#L219-L283)
- [app.go:113-119](file://server/api/internal/app/app.go#L113-L119)

## 核心组件

### 前端用户页面组件

用户管理功能的前端实现位于 `web/src/features/users/users-page.tsx` 文件中，这是一个 React 客户端组件，提供了完整的用户管理界面。

**主要特性：**
- 实时用户数据查询和展示
- 搜索和过滤功能
- 用户详情面板
- **用户事件时间线展示**（新增）
- 项目选择器集成
- 响应式设计支持

### 后端处理器架构

后端的用户管理逻辑由多个处理器结构体实现，包括治理处理器和分析处理器：

**治理处理器 (GovernanceHandler)**：
- `/projects/:id/users` - 获取项目用户列表
- `/projects/:id/users/:distinct_id/profile` - 获取用户画像
- `/projects/:id/identities` - 查询身份映射
- `/projects/:id/properties` - 获取属性定义

**分析处理器 (AnalyticsHandler)**：
- **`/projects/:id/users/:distinct_id/events`** - 获取用户事件时间线（新增）

### 数据库架构

用户管理功能依赖于 PostgreSQL 和 ClickHouse 的混合存储架构：

```mermaid
erDiagram
PROPERTY_DEFINITIONS {
int64 id PK
string name
string display_name
string data_type
string scope
string description
int16 status
timestamp first_seen
timestamp last_seen
}
IDENTITY_MAPPINGS {
int64 id PK
string anonymous_id
string user_id
timestamp first_seen
timestamp last_seen
timestamp updated_at
}
USERS {
string distinct_id PK
string user_id
string anonymous_id
jsonb properties
timestamp updated_at
uint32 project_id
}
EVENTS {
string event
string distinct_id
string user_id
string anonymous_id
timestamp time
string lib
string os
jsonb properties
uint32 project_id
}
PROPERTY_DEFINITIONS ||--o{ USERS : "定义属性"
IDENTITY_MAPPINGS ||--o{ USERS : "映射身份"
EVENTS ||--o{ USERS : "记录事件"
```

**图表来源**
- [20260617_governance_metadata.sql](file://deploy/migrations/postgres/20260617_governance_metadata.sql)

**章节来源**
- [users-page.tsx:21-30](file://web/src/features/users/users-page.tsx#L21-L30)
- [governance.go:14-25](file://server/api/internal/handler/governance.go#L14-L25)
- [analytics.go:219-283](file://server/api/internal/handler/analytics.go#L219-L283)
- [20260617_governance_metadata.sql](file://deploy/migrations/postgres/20260617_governance_metadata.sql)

## 架构概览

用户管理功能采用分层架构设计，确保了良好的可维护性和扩展性：

```mermaid
graph TD
subgraph "表现层"
Client[React 组件]
UI[用户界面]
EventTimeline[事件时间线]
end
subgraph "应用层"
API[API 服务]
Handler[处理器]
AnalyticsHandler[分析处理器]
end
subgraph "服务层"
Governance[治理服务]
Analytics[分析服务]
Project[项目服务]
end
subgraph "数据访问层"
PG[PostgreSQL 连接池]
CH[ClickHouse 连接]
Cache[缓存层]
end
Client --> API
API --> Handler
API --> AnalyticsHandler
Handler --> Governance
AnalyticsHandler --> Analytics
Governance --> PG
Analytics --> CH
PG --> Cache
CH --> Cache
```

**图表来源**
- [app.go:113-119](file://server/api/internal/app/app.go#L113-L119)
- [governance.go:14-18](file://server/api/internal/handler/governance.go#L14-L18)
- [analytics.go:219-220](file://server/api/internal/handler/analytics.go#L219-L220)

## 详细组件分析

### 用户页面组件分析

用户页面组件是一个功能完整的 React 组件，实现了用户管理的所有核心功能：

```mermaid
classDiagram
class UsersPage {
+useState projectId
+useState query
+useState submittedQuery
+useState selected
+useEffect projects
+useQuery users
+render() JSX.Element
}
class UserProfile {
+string distinct_id
+string user_id
+string anonymous_id
+map[string]interface{} properties
+time updated_at
}
class IdentityMapping {
+string anonymous_id
+string user_id
+time first_seen
+time last_seen
+time updated_at
}
class UserEvent {
+string event
+string distinct_id
+string user_id
+string anonymous_id
+string time
+string lib
+string os
+map[string]interface{} properties
}
UsersPage --> UserProfile : "显示"
UsersPage --> IdentityMapping : "查询"
UsersPage --> UserEvent : "时间线"
```

**图表来源**
- [users-page.tsx:21-30](file://web/src/features/users/users-page.tsx#L21-L30)
- [governance.go:143-149](file://server/api/internal/handler/governance.go#L143-L149)
- [api.ts:72-81](file://web/src/lib/api.ts#L72-L81)

#### 用户查询流程

用户查询功能通过以下流程实现：

```mermaid
sequenceDiagram
participant Client as "客户端"
participant API as "API 服务"
participant Handler as "治理处理器"
participant PG as "PostgreSQL"
participant CH as "ClickHouse"
Client->>API : GET /v1/projects/{id}/users
API->>Handler : users()
Handler->>PG : 查询用户列表
PG-->>Handler : 用户数据
Handler->>CH : 查询用户画像
CH-->>Handler : 画像数据
Handler-->>API : JSON 响应
API-->>Client : 用户列表
Note over Client,CH : 用户查询完成
```

**图表来源**
- [governance.go:120-160](file://server/api/internal/handler/governance.go#L120-L160)

#### 身份映射查询流程

身份映射查询支持多种过滤条件：

```mermaid
flowchart TD
Start([开始查询]) --> GetParams["获取查询参数"]
GetParams --> CheckUserID{"是否指定 user_id?"}
CheckUserID --> |是| AddUserID["添加 user_id 条件"]
CheckUserID --> |否| CheckAnonymousID{"是否指定 anonymous_id?"}
AddUserID --> CheckAnonymousID
CheckAnonymousID --> |是| AddAnonymousID["添加 anonymous_id 条件"]
CheckAnonymousID --> |否| SetLimit["设置默认限制"]
AddAnonymousID --> SetLimit
SetLimit --> BuildQuery["构建 SQL 查询"]
BuildQuery --> ExecuteQuery["执行查询"]
ExecuteQuery --> ParseResults["解析查询结果"]
ParseResults --> ReturnResults["返回映射列表"]
ReturnResults --> End([结束])
```

**图表来源**
- [governance.go:74-93](file://server/api/internal/handler/governance.go#L74-L93)

**章节来源**
- [users-page.tsx:21-30](file://web/src/features/users/users-page.tsx#L21-L30)
- [governance.go:74-160](file://server/api/internal/handler/governance.go#L74-L160)

### 治理处理器分析

治理处理器是用户管理功能的核心后端组件，负责处理所有用户相关的 API 请求：

```mermaid
classDiagram
class GovernanceHandler {
+PG pgxpool.Pool
+CH driver.Conn
+Register(router) void
+properties(context) void
+identities(context) void
+users(context) void
+profile(context) void
-parseJSONProps(raw) map
-clampLimit(raw, min, max) int
}
class UserProfile {
+string distinct_id
+string user_id
+string anonymous_id
+map[string]interface{} properties
+time updated_at
}
class PropertyDef {
+int64 id
+string name
+string display_name
+string data_type
+string scope
+string description
+int16 status
+time first_seen
+time last_seen
}
GovernanceHandler --> UserProfile : "创建"
GovernanceHandler --> PropertyDef : "创建"
```

**图表来源**
- [governance.go:14-25](file://server/api/internal/handler/governance.go#L14-L25)
- [governance.go:53-72](file://server/api/internal/handler/governance.go#L53-L72)

#### API 接口设计

治理处理器提供以下 RESTful API 接口：

| 接口 | 方法 | 描述 | 参数 |
|------|------|------|------|
| `/projects/:id/properties` | GET | 获取属性定义 | scope(event,user) |
| `/projects/:id/identities` | GET | 查询身份映射 | user_id, anonymous_id, limit |
| `/projects/:id/users` | GET | 获取用户列表 | - |
| `/projects/:id/users/:distinct_id/profile` | GET | 获取用户画像 | - |

**章节来源**
- [governance.go:20-25](file://server/api/internal/handler/governance.go#L20-L25)
- [governance.go:27-72](file://server/api/internal/handler/governance.go#L27-L72)

### 数据模型分析

用户管理功能涉及多个核心数据表，每个表都有特定的用途和关系：

#### 属性定义表 (property_definitions)

属性定义表存储用户和事件属性的元数据信息：

| 字段名 | 类型 | 描述 | 约束 |
|--------|------|------|------|
| id | int64 | 主键 | PRIMARY KEY |
| name | string | 属性名称 | NOT NULL |
| display_name | string | 显示名称 | DEFAULT '' |
| data_type | string | 数据类型 | NOT NULL |
| scope | string | 作用域 | NOT NULL |
| description | string | 描述信息 | DEFAULT '' |
| status | int16 | 状态 | NOT NULL |
| first_seen | timestamp | 首次出现时间 | |
| last_seen | timestamp | 最后出现时间 | |

#### 身份映射表 (identity_mappings)

身份映射表记录匿名用户和已登录用户之间的关联关系：

| 字段名 | 类型 | 描述 | 约束 |
|--------|------|------|------|
| id | int64 | 主键 | PRIMARY KEY |
| anonymous_id | string | 匿名用户ID | NOT NULL |
| user_id | string | 用户ID | NOT NULL |
| first_seen | timestamp | 首次关联时间 | |
| last_seen | timestamp | 最后关联时间 | |
| updated_at | timestamp | 更新时间 | NOT NULL |

#### 用户表 (users)

用户表存储用户的最终识别信息和属性：

| 字段名 | 类型 | 描述 | 约束 |
|--------|------|------|------|
| distinct_id | string | 最终用户ID | PRIMARY KEY |
| user_id | string | 已登录用户ID | |
| anonymous_id | string | 匿名用户ID | |
| properties | jsonb | 用户属性 | NOT NULL |
| updated_at | timestamp | 更新时间 | NOT NULL |
| project_id | uint32 | 项目ID | NOT NULL |

#### 事件表 (events)

**新增**：事件表存储用户行为事件数据：

| 字段名 | 类型 | 描述 | 约束 |
|--------|------|------|------|
| event | string | 事件名称 | NOT NULL |
| distinct_id | string | 用户ID | NOT NULL |
| user_id | string | 登录用户ID | |
| anonymous_id | string | 匿名用户ID | |
| time | timestamp | 事件时间 | NOT NULL |
| lib | string | 事件库标识 | |
| os | string | 操作系统信息 | |
| properties | jsonb | 事件属性 | NOT NULL |
| project_id | uint32 | 项目ID | NOT NULL |

**章节来源**
- [20260617_governance_metadata.sql](file://deploy/migrations/postgres/20260617_governance_metadata.sql)

## 新增功能：用户事件时间线

### 功能概述

用户事件时间线功能是本次更新的重要组成部分，它允许分析师深入查看单个用户的行为轨迹，包括用户在特定时间范围内的所有事件活动。

### 前端组件实现

用户事件时间线功能通过 `ProfileSheet` 组件中的 `userEvents` 查询实现：

```mermaid
flowchart TD
Start([打开用户详情]) --> InitState["初始化时间范围和过滤器"]
InitState --> SetDefaultRange["设置默认时间范围"]
SetDefaultRange --> QueryEvents["调用 userEvents API"]
QueryEvents --> Loading["显示加载状态"]
Loading --> ReceiveData["接收事件数据"]
ReceiveData --> RenderTimeline["渲染时间线"]
RenderTimeline --> FilterEvents["事件过滤"]
FilterEvents --> DisplayResult["显示结果"]
DisplayResult --> End([完成])
QueryEvents --> Error["错误处理"]
Error --> ShowEmpty["显示空状态"]
ShowEmpty --> End
```

**图表来源**
- [users-page.tsx:241-251](file://web/src/features/users/users-page.tsx#L241-L251)
- [users-page.tsx:273-307](file://web/src/features/users/users-page.tsx#L273-L307)

### API 接口设计

新增的用户事件查询接口提供灵活的时间范围和事件过滤功能：

**接口定义**：
- **路径**：`/v1/projects/:id/users/:distinct_id/events`
- **方法**：GET
- **参数**：
  - `from` (可选)：开始时间戳（毫秒）
  - `to` (可选)：结束时间戳（毫秒）
  - `event` (可选)：事件名称过滤
  - `limit` (可选，默认100)：最大返回数量（1-500）

**响应格式**：
```json
{
  "data": [
    {
      "event": "user_login",
      "distinct_id": "user_123",
      "user_id": "user_123",
      "anonymous_id": "anon_456",
      "time": "2026-06-17T10:30:00Z",
      "lib": "web-sdk",
      "os": "Windows 10",
      "properties": {
        "ip": "192.168.1.1",
        "user_agent": "Chrome 126"
      }
    }
  ]
}
```

### 后端实现分析

分析处理器的 `userEvents` 方法实现了完整的事件查询逻辑：

```mermaid
sequenceDiagram
participant Client as "客户端"
participant API as "API 服务"
participant Handler as "分析处理器"
participant CH as "ClickHouse"
Client->>API : GET /v1/projects/{id}/users/{distinct_id}/events
API->>Handler : userEvents()
Handler->>Handler : 验证参数
Handler->>Handler : 设置默认时间范围
Handler->>CH : 执行事件查询
CH-->>Handler : 返回事件数据
Handler->>Handler : 解析 JSON 属性
Handler-->>API : 返回格式化数据
API-->>Client : 用户事件时间线
```

**图表来源**
- [analytics.go:219-283](file://server/api/internal/handler/analytics.go#L219-L283)

### 时间线展示组件

事件时间线通过 `EventTimelineItem` 组件实现美观的可视化展示：

```mermaid
classDiagram
class EventTimelineItem {
+props item : UserEvent
+render() JSX.Element
}
class TimelineItem {
+props title : string
+props description : string
+props time : string
+render() JSX.Element
}
class UserEvent {
+string event
+string time
+string lib
+string os
+object properties
}
EventTimelineItem --> TimelineItem : "包装"
TimelineItem --> UserEvent : "使用"
```

**图表来源**
- [users-page.tsx:391-416](file://web/src/features/users/users-page.tsx#L391-L416)
- [api.ts:72-81](file://web/src/lib/api.ts#L72-L81)

**章节来源**
- [users-page.tsx:241-307](file://web/src/features/users/users-page.tsx#L241-L307)
- [users-page.tsx:391-416](file://web/src/features/users/users-page.tsx#L391-L416)
- [api.ts:152-165](file://web/src/lib/api.ts#L152-L165)
- [analytics.go:219-283](file://server/api/internal/handler/analytics.go#L219-L283)

## 依赖关系分析

用户管理功能的依赖关系体现了清晰的分层架构：

```mermaid
graph LR
subgraph "外部依赖"
Gin[Gin Web框架]
PGX[PGX PostgreSQL驱动]
ClickHouse[ClickHouse Go驱动]
ReactQuery[TanStack React Query]
Dayjs[Day.js 时间库]
Lucide[Lucide 图标库]
end
subgraph "内部模块"
UsersPage[用户页面组件]
ProfileSheet[用户详情面板]
EventTimeline[事件时间线组件]
GovernanceHandler[治理处理器]
AnalyticsHandler[分析处理器]
APIApp[API应用]
APIService[API服务]
end
UsersPage --> ReactQuery
ProfileSheet --> Dayjs
EventTimeline --> Lucide
UsersPage --> APIApp
APIApp --> GovernanceHandler
APIApp --> AnalyticsHandler
AnalyticsHandler --> PGX
AnalyticsHandler --> ClickHouse
APIApp --> Gin
```

**图表来源**
- [users-page.tsx:4-6](file://web/src/features/users/users-page.tsx#L4-L6)
- [users-page.tsx:382-389](file://web/src/features/users/users-page.tsx#L382-L389)
- [governance.go:3-12](file://server/api/internal/handler/governance.go#L3-L12)
- [analytics.go:3-12](file://server/api/internal/handler/analytics.go#L3-L12)
- [app.go:113-119](file://server/api/internal/app/app.go#L113-L119)

**章节来源**
- [users-page.tsx:4-6](file://web/src/features/users/users-page.tsx#L4-L6)
- [users-page.tsx:382-389](file://web/src/features/users/users-page.tsx#L382-L389)
- [governance.go:3-12](file://server/api/internal/handler/governance.go#L3-L12)
- [analytics.go:3-12](file://server/api/internal/handler/analytics.go#L3-L12)
- [app.go:113-119](file://server/api/internal/app/app.go#L113-L119)

## 性能考虑

用户管理功能在设计时充分考虑了性能优化，新增的用户事件时间线功能也包含了专门的优化策略：

### 查询优化策略

1. **索引优化**
   - 在 `property_definitions` 表上建立复合索引以加速属性查询
   - 在 `identity_mappings` 表上优化查询条件字段索引
   - 在 `users` 表上使用适当的分区策略
   - **新增**：在 `events` 表上建立时间范围查询索引

2. **缓存策略**
   - 使用 Redis 缓存常用的属性定义
   - 实现查询结果缓存机制
   - 支持响应式缓存失效
   - **新增**：缓存用户事件查询结果

3. **连接池管理**
   - PostgreSQL 使用连接池避免频繁连接
   - ClickHouse 连接复用优化
   - 连接超时和重试机制
   - **新增**：事件查询专用连接池

### 前端性能优化

1. **懒加载**
   - 用户详情面板按需加载
   - 分页加载大量用户数据
   - 图表组件延迟渲染
   - **新增**：事件时间线按需加载

2. **状态管理**
   - 使用 React Query 管理服务器状态
   - 实现智能缓存策略
   - 支持自动刷新和手动刷新
   - **新增**：时间线查询状态管理

3. **时间线优化**
   - **新增**：虚拟滚动支持大量事件显示
   - **新增**：事件过滤防抖机制
   - **新增**：时间范围选择优化

### 数据库查询优化

1. **事件查询优化**
   - 使用 `ORDER BY time DESC` 确保最新事件优先
   - 实施 `LIMIT` 限制返回数量
   - **新增**：时间范围预过滤减少扫描量

2. **索引策略**
   - **新增**：`events` 表复合索引 `(project_id, distinct_id, time)`
   - **新增**：事件名称过滤索引
   - **新增**：时间范围查询优化

**章节来源**
- [analytics.go:219-283](file://server/api/internal/handler/analytics.go#L219-L283)
- [users-page.tsx:241-251](file://web/src/features/users/users-page.tsx#L241-L251)

## 故障排除指南

### 常见问题及解决方案

#### 用户数据查询失败

**症状：** 用户列表无法加载或显示错误

**可能原因：**
1. PostgreSQL 连接异常
2. 查询参数不正确
3. 数据库权限不足

**解决步骤：**
1. 检查数据库连接状态
2. 验证项目ID的有效性
3. 确认用户权限设置

#### 用户画像获取错误

**症状：** 用户详情页面显示 "未找到"

**可能原因：**
1. 用户ID不存在
2. ClickHouse 查询超时
3. 数据同步延迟

**解决步骤：**
1. 验证用户ID格式
2. 检查数据同步状态
3. 查看查询日志

#### 身份映射查询异常

**症状：** 身份映射结果不准确

**可能原因：**
1. 查询条件冲突
2. 数据重复
3. 时间戳问题

**解决步骤：**
1. 清晰化查询条件
2. 检查数据去重逻辑
3. 验证时间戳一致性

#### **新增**：用户事件时间线查询问题

**症状：** 事件时间线无法加载或显示为空

**可能原因：**
1. **新增**：时间范围设置不当
2. **新增**：事件过滤条件过于严格
3. **新增**：ClickHouse 查询超时
4. **新增**：网络连接问题

**解决步骤：**
1. **新增**：调整时间范围到合理区间
2. **新增**：清除事件名称过滤器测试
3. **新增**：检查 ClickHouse 连接状态
4. **新增**：验证网络连接稳定性

**章节来源**
- [governance.go:47-49](file://server/api/internal/handler/governance.go#L47-L49)
- [governance.go:182-184](file://server/api/internal/handler/governance.go#L182-L184)
- [analytics.go:219-283](file://server/api/internal/handler/analytics.go#L219-L283)

## 结论

AeroLog 的用户管理功能展现了现代数据分析平台的最佳实践。通过前后端分离架构、清晰的分层设计和完善的性能优化策略，该功能能够高效地处理大规模用户数据的查询和管理需求。

**重要更新**：新增的用户事件时间线功能显著增强了用户管理能力，为分析师提供了深入洞察个体用户行为轨迹的能力。这一功能的引入标志着 AeroLog 平台从简单的用户统计向深度用户行为分析的跨越。

### 主要优势

1. **架构清晰** - 分层设计确保了代码的可维护性和可扩展性
2. **性能优异** - 多种优化策略保证了系统的高性能运行
3. **用户体验好** - 响应式界面和实时数据更新提升了用户体验
4. **数据安全** - 完善的权限控制和数据保护机制
5. ****分析能力强** - **新增**：深度用户行为追踪能力

### 未来改进方向

1. **增强搜索功能** - 支持更复杂的用户属性搜索
2. **批量操作** - 添加用户数据的批量管理功能
3. **审计日志** - 记录重要的用户管理操作
4. **API 扩展** - 支持更多的用户管理 API 接口
5. ****分析工具增强** - **新增**：用户行为模式分析工具
6. ****实时监控** - **新增**：用户事件实时监控告警

该用户管理功能为 AeroLog 平台提供了坚实的基础，能够满足各种规模企业的用户数据分析和管理需求。新增的用户事件时间线功能进一步巩固了平台在用户行为分析领域的领先地位。