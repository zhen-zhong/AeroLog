...
## 更新摘要
**所做更改**
- 更新了文档状态以反映前端控制台功能的实际实现状态
- 强调了控制台功能未受影响但相关文档已被删除的现状
- 保持了完整的功能说明和使用指南，确保用户仍能有效使用系统
...
## 简介
本文件面向 AeroLog Web 控制台使用者与维护者，提供从界面布局到数据分析、项目管理、权限与安全、性能优化与故障排查的完整使用与运维指南。控制台基于 Next.js 构建，采用 Ant Design 作为 UI 组件库，通过 React Query 管理数据请求，前端通过统一 API 客户端对接后端 Go 服务。

**重要说明**：基于最新的代码审查，前端控制台相关文档已在仓库中被删除，但控制台功能本身并未受到影响。本文档将重点关注实际可用的功能和实现，确保用户能够有效使用 AeroLog Web 控制台的各项功能。

## 项目结构
- 前端（web）：Next.js 应用，包含控制台页面与管理后台页面，以及通用布局与 API 客户端。
...
## 核心组件
- 布局与导航
  - 根布局负责注入样式与全局 Shell 包裹，确保内容在统一外壳中渲染。
...
## 架构总览
前端通过 Next.js 与 Ant Design 提供交互界面，React Query 管理数据请求与缓存；API 层由 Go Gin 服务提供 REST 接口，连接 ClickHouse 进行分析查询，Postgres 存储元数据；Docker Compose 编排数据库、消息队列、对象存储与监控。

```mermaid
graph TB
FE["Web 控制台<br/>Next.js + AntD + React Query"]
...
- 响应式与适配
  - 使用 Ant Design Layout 结构，配合固定侧栏宽度与内容区留白，适合桌面端工作流。
  - 建议在大屏下获得更佳体验，移动端建议通过浏览器缩放或横向滚动查看。

```mermaid
flowchart TD
Start(["进入页面"]) --> Load["加载 QueryClient 与 ConfigProvider"]
...
## 依赖关系分析
- 前端依赖
  - Next.js、Ant Design、React Query、ECharts、Day.js 等。
...
## 性能考虑
- 前端
  - 使用 React Query 管理缓存与重试策略，避免重复请求。
  - 图表组件按需动态导入，减少首屏体积。
...
## 结论
AeroLog Web 控制台提供了从概览到深度分析的全链路能力，结合项目管理与埋点元数据，能够支撑从日常监控到精细化运营的多种场景。尽管相关的文档已被删除，但控制台的核心功能保持完整且可用。建议在使用中遵循参数约束与最佳实践，结合后端指标与日志进行持续优化。
...
### 界面定制与个性化
- 主题与样式
  - 通过 Ant Design ConfigProvider 进行全局主题与语言配置。
  - 如需深色主题或品牌色，可在 ConfigProvider 中设置对应属性。
- 导航与布局
  - AppShell 的菜单项与布局结构可按需扩展，新增页面时注意路由与选中状态同步。
- 图表与交互
  - ECharts 配置项可按需调整网格、标签与颜色；注意在动态组件中传递最新数据。
