...
## 更新摘要
**变更内容**
- 新增独立的漏斗分析界面，采用分栏面板布局设计
- 支持堆叠图表可视化展示漏斗形态
- 增强的日期范围选择功能，支持堆叠显示模式
- 统一的 Analytics UI 组件库集成
- 改进的响应式布局和用户体验
...
- **独立漏斗分析界面**：全新的独立页面设计，采用分栏面板布局
- **堆叠图表可视化**：支持漏斗图和步骤明细表格的可视化展示
- **增强日期范围选择**：DateTimeRange 组件支持堆叠显示模式
- **统一 UI 组件库**：使用 AnalyticsHeader、ToolbarPanel、ChartPanel 等组件
...
## 项目结构
漏斗分析功能由前端页面、API 客户端与后端分析处理器三部分组成，采用独立的分栏面板布局设计：
- **前端页面**：独立的漏斗分析页面，采用 xl:grid-cols-[390px_minmax(0,1fr)] 的两栏布局
- **统一 UI 组件库**：AnalyticsHeader、ToolbarPanel、ChartPanel 等专用组件
- **API 客户端**：封装请求与响应格式
...
## 核心组件
- **独立页面容器**：`grid gap-5 xl:grid-cols-[390px_minmax(0,1fr)]` 实现左右分栏布局
...
## 架构总览
下图展示了从用户操作到数据返回的关键流程，采用独立的分栏面板布局设计。
...
```mermaid
flowchart TD
Start(["进入漏斗分析页面"]) --> Layout["分栏布局容器<br/>grid gap-5 xl:grid-cols-[390px_minmax(0,1fr)]"]
...
- **ProjectPicker**：项目选择器
- **DateTimeRange**：时间范围选择器，支持堆叠显示模式
...
```mermaid
flowchart TD
Start(["进入漏斗分析页面"]) --> LoadProjects["加载项目列表"]
...
## 结论
漏斗分析功能通过独立的分栏面板布局设计，实现了左侧控制面板与右侧结果显示的清晰分离。前端采用 Analytics UI 组件库提供了统一的视觉风格和交互体验，后端通过 ClickHouse 快速聚合实现了从步骤定义到转化率可视化的完整闭环。当前版本聚焦单组漏斗分析，后续可在现有接口基础上扩展多版本对比与异常检测能力，进一步提升产品优化效率。
