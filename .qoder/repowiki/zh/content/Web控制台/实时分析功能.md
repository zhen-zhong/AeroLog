# 实时分析功能

<cite>
**本文档引用的文件**
- [web/src/app/console/realtime/page.tsx](file://web/src/app/console/realtime/page.tsx)
- [server/api/internal/handler/analytics.go](file://server/api/internal/handler/analytics.go)
- [server/consumer/internal/chsink/sink.go](file://server/consumer/internal/chsink/sink.go)
- [sdk/web/src/index.ts](file://sdk/web/src/index.ts)
- [server/pkg/model/event.go](file://server/pkg/model/event.go)
- [server/collector/cmd/main.go](file://server/collector/cmd/main.go)
- [server/consumer/cmd/main.go](file://server/consumer/cmd/main.go)
- [web/src/lib/api.ts](file://web/src/lib/api.ts)
- [deploy/docker-compose.yml](file://deploy/docker-compose.yml)
- [server/consumer/internal/worker/worker.go](file://server/consumer/internal/worker/worker.go)
- [server/collector/internal/handler/track.go](file://server/collector/internal/handler/track.go)
- [server/consumer/internal/config/config.go](file://server/consumer/internal/config/config.go)
- [server/api/internal/config/config.go](file://server/api/internal/config/config.go)
- [sdk/web/src/storage.ts](file://sdk/web/src/storage.ts)
- [server/consumer/internal/etl/etl.go](file://server/consumer/internal/etl/etl.go)
</cite>

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构概览](#架构概览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考虑](#性能考虑)
8. [故障排除指南](#故障排除指南)
9. [结论](#结论)

## 简介

AeroLog 是一个实时事件分析系统，提供从事件采集、传输、存储到分析展示的完整解决方案。该系统采用现代微服务架构，支持高并发实时数据处理，能够满足企业级实时分析需求。

系统的核心特性包括：
- 实时事件采集与传输
- 高性能数据存储与查询
- 多维度数据分析功能
- 用户友好的可视化界面
- 完善的错误处理与监控机制

## 项目结构

AeroLog 采用模块化的项目结构，主要包含以下核心模块：

```mermaid
graph TB
subgraph "前端层"
WEB[Web应用<br/>Next.js]
SDK[Web SDK<br/>事件采集]
end
subgraph "后端服务层"
COLLECTOR[Collector<br/>事件收集]
CONSUMER[Consumer<br/>数据处理]
API[API Server<br/>数据分析]
end
subgraph "数据存储层"
KAFKA[Kafka<br/>消息队列]
CLICKHOUSE[ClickHouse<br/>OLAP数据库]
POSTGRES[Postgres<br/>元数据存储]
REDIS[Redis<br/>缓存]
end
subgraph "监控层"
PROMETHEUS[Prometheus<br/>指标收集]
GRAFANA[Grafana<br/>数据可视化]
end
WEB --> API
SDK --> COLLECTOR
COLLECTOR --> KAFKA
KAFKA --> CONSUMER
CONSUMER --> CLICKHOUSE
CONSUMER --> POSTGRES
API --> CLICKHOUSE
API --> POSTGRES
COLLECTOR --> REDIS
CONSUMER --> PROMETHEUS
API --> PROMETHEUS
PROMETHEUS --> GRAFANA
```

**图表来源**
- [deploy/docker-compose.yml:1-147](file://deploy/docker-compose.yml#L1-L147)
- [server/collector/cmd/main.go:1-26](file://server/collector/cmd/main.go#L1-L26)
- [server/consumer/cmd/main.go:1-26](file://server/consumer/cmd/main.go#L1-L26)

**章节来源**
- [deploy/docker-compose.yml:1-147](file://deploy/docker-compose.yml#L1-L147)
- [web/src/app/console/realtime/page.tsx:1-135](file://web/src/app/console/realtime/page.tsx#L1-L135)

## 核心组件

### 实时分析页面组件

实时分析功能的核心是前端的实时分析页面，该页面提供了直观的实时数据展示界面：

```mermaid
classDiagram
class RealtimePage {
+useState projectId
+useState range
+useEffect setup
+useQuery topEvents
+render() ReactElement
}
class AnalyticsHeader {
+title string
+description string
+action ReactNode
}
class ReportControls {
+projects Project[]
+projectId number
+range [Dayjs, Dayjs]
+onProjectChange function
+onRangeChange function
}
class MetricTile {
+label string
+value number
+hint string
+loading boolean
}
class EventRankList {
+events TopEvent[]
+active string
+onSelect function
+loading boolean
}
RealtimePage --> AnalyticsHeader
RealtimePage --> ReportControls
RealtimePage --> MetricTile
RealtimePage --> EventRankList
```

**图表来源**
- [web/src/app/console/realtime/page.tsx:20-135](file://web/src/app/console/realtime/page.tsx#L20-L135)

### 事件采集SDK

Web SDK 实现了完整的事件采集功能，支持多种上报策略：

```mermaid
classDiagram
class AeroLog {
-cfg AeroLogConfig
-store EventStore
-buffer AeroEvent[]
-flushing boolean
-retryAttempt number
-timer number
+track(event, properties)
+identify(userId)
+flush() Promise~void~
+send(events) Promise~boolean~
+drainBuffer() Promise~void~
+uploadFromStore() Promise~void~
}
class EventStore {
-dbPromise Promise~IDBDatabase~
-memFallback StoredEvent[]
-nextMemId number
+add(payload) Promise~void~
+take(n) Promise~StoredEvent[]~
+remove(ids) Promise~void~
}
class BackoffStrategy {
+backoffMs(attempt) number
+exponentialBackoff()
+linearBackoff()
}
AeroLog --> EventStore
AeroLog --> BackoffStrategy
```

**图表来源**
- [sdk/web/src/index.ts:16-307](file://sdk/web/src/index.ts#L16-L307)
- [sdk/web/src/storage.ts:16-141](file://sdk/web/src/storage.ts#L16-L141)

**章节来源**
- [web/src/app/console/realtime/page.tsx:1-135](file://web/src/app/console/realtime/page.tsx#L1-L135)
- [sdk/web/src/index.ts:1-307](file://sdk/web/src/index.ts#L1-L307)
- [sdk/web/src/storage.ts:1-141](file://sdk/web/src/storage.ts#L1-L141)

## 架构概览

AeroLog 采用事件驱动的实时数据处理架构，整个系统按照数据流向可以分为以下几个阶段：

```mermaid
sequenceDiagram
participant Client as 客户端浏览器
participant SDK as Web SDK
participant Collector as Collector服务
participant Kafka as Kafka消息队列
participant Consumer as Consumer服务
participant ClickHouse as ClickHouse数据库
participant API as API服务
participant Frontend as 前端应用
Client->>SDK : 用户操作触发事件
SDK->>SDK : 缓冲事件到内存
SDK->>SDK : 超时或批量触发上报
SDK->>Collector : POST /v1/track
Collector->>Collector : 验证事件格式
Collector->>Kafka : 发送事件消息
Kafka-->>Consumer : 消费事件消息
Consumer->>Consumer : 解析事件并富化
Consumer->>ClickHouse : 批量写入事件数据
Frontend->>API : GET /analytics/top_events
API->>ClickHouse : 查询实时事件统计
ClickHouse-->>API : 返回查询结果
API-->>Frontend : 响应实时数据
Frontend->>Frontend : 更新UI显示
```

**图表来源**
- [server/collector/internal/handler/track.go:60-133](file://server/collector/internal/handler/track.go#L60-L133)
- [server/consumer/internal/worker/worker.go:94-156](file://server/consumer/internal/worker/worker.go#L94-L156)
- [server/api/internal/handler/analytics.go:76-112](file://server/api/internal/handler/analytics.go#L76-L112)

**章节来源**
- [server/collector/internal/handler/track.go:1-211](file://server/collector/internal/handler/track.go#L1-L211)
- [server/consumer/internal/worker/worker.go:1-211](file://server/consumer/internal/worker/worker.go#L1-L211)
- [server/api/internal/handler/analytics.go:1-297](file://server/api/internal/handler/analytics.go#L1-L297)

## 详细组件分析

### 实时数据查询流程

实时分析功能的核心是实时数据查询，该流程确保用户能够获得最新的事件统计数据：

```mermaid
flowchart TD
Start([开始实时查询]) --> SetParams[设置查询参数<br/>from, to, limit]
SetParams --> CallAPI[调用API接口<br/>GET /analytics/top_events]
CallAPI --> ValidateParams{验证参数<br/>from, to, limit}
ValidateParams --> |参数无效| ReturnError[返回错误信息]
ValidateParams --> |参数有效| BuildQuery[构建ClickHouse查询]
BuildQuery --> ExecuteQuery[执行查询]
ExecuteQuery --> ProcessResults[处理查询结果<br/>事件计数, 用户去重]
ProcessResults --> FormatResponse[格式化响应数据]
FormatResponse --> UpdateUI[更新前端界面]
UpdateUI --> ScheduleRefresh[安排下次刷新]
ScheduleRefresh --> SetParams
ReturnError --> End([结束])
End([结束])
```

**图表来源**
- [web/src/app/console/realtime/page.tsx:36-43](file://web/src/app/console/realtime/page.tsx#L36-L43)
- [web/src/lib/api.ts:113-124](file://web/src/lib/api.ts#L113-L124)
- [server/api/internal/handler/analytics.go:76-112](file://server/api/internal/handler/analytics.go#L76-L112)

### 事件采集与上报机制

Web SDK 实现了多阶段的事件上报机制，确保数据的可靠传输：

```mermaid
flowchart TD
UserAction[用户操作] --> BufferEvent[缓冲事件到内存]
BufferEvent --> CheckBatch{检查批次大小}
CheckBatch --> |达到批次阈值| FlushBatch[触发批量上报]
CheckBatch --> |未达到阈值| WaitTimeout[等待超时]
WaitTimeout --> FlushTimeout[触发定时上报]
FlushBatch --> SendRequest[发送HTTP请求]
FlushTimeout --> SendRequest
SendRequest --> CheckResponse{检查响应状态}
CheckResponse --> |成功| ClearBuffer[清空缓冲区]
CheckResponse --> |失败| StoreToDisk[存储到IndexedDB]
StoreToDisk --> RetryMechanism[重试机制]
RetryMechanism --> UploadFromStore[从磁盘上传]
UploadFromStore --> CheckOnline{检查网络状态}
CheckOnline --> |在线| SendRequest
CheckOnline --> |离线| WaitOnline[等待网络恢复]
WaitOnline --> CheckOnline
ClearBuffer --> End([完成])
WaitTimeout --> End
WaitOnline --> End
```

**图表来源**
- [sdk/web/src/index.ts:116-145](file://sdk/web/src/index.ts#L116-L145)
- [sdk/web/src/storage.ts:46-94](file://sdk/web/src/storage.ts#L46-L94)

**章节来源**
- [sdk/web/src/index.ts:1-307](file://sdk/web/src/index.ts#L1-L307)
- [sdk/web/src/storage.ts:1-141](file://sdk/web/src/storage.ts#L1-L141)

### 数据处理与存储流程

Consumer 服务负责从 Kafka 消费事件并进行数据处理和存储：

```mermaid
sequenceDiagram
participant Kafka as Kafka
participant Consumer as Consumer
participant ETL as ETL处理
participant Sink as 数据库写入
participant ClickHouse as ClickHouse
Kafka->>Consumer : 消费事件消息
Consumer->>Consumer : 解析EnvelopedEvent
Consumer->>ETL : 事件富化处理
ETL->>ETL : 解析UA信息
ETL->>ETL : 解析地理位置
ETL->>ETL : 校验事件格式
Consumer->>Sink : 分离事件类型
Sink->>Sink : 组装批量写入
Sink->>ClickHouse : INSERT INTO events_buffer
ClickHouse-->>Sink : 写入确认
Sink-->>Consumer : 处理完成
Consumer->>Consumer : 标记消息偏移量
```

**图表来源**
- [server/consumer/internal/worker/worker.go:158-192](file://server/consumer/internal/worker/worker.go#L158-L192)
- [server/consumer/internal/chsink/sink.go:49-107](file://server/consumer/internal/chsink/sink.go#L49-L107)
- [server/consumer/internal/etl/etl.go:29-89](file://server/consumer/internal/etl/etl.go#L29-L89)

**章节来源**
- [server/consumer/internal/worker/worker.go:1-211](file://server/consumer/internal/worker/worker.go#L1-L211)
- [server/consumer/internal/chsink/sink.go:1-333](file://server/consumer/internal/chsink/sink.go#L1-L333)
- [server/consumer/internal/etl/etl.go:1-90](file://server/consumer/internal/etl/etl.go#L1-L90)

## 依赖关系分析

系统各组件之间的依赖关系如下所示：

```mermaid
graph TB
subgraph "前端依赖"
WEB_DEPS[Next.js应用]
REACT_QUERY[React Query]
LUCIDE[Lucide图标]
DAYJS[Day.js日期库]
end
subgraph "后端依赖"
GIN[Gin框架]
CLICKHOUSE[ClickHouse驱动]
KAFKA[Kafka客户端]
PGX[Postgres连接池]
SARAMA[Sarama Kafka库]
end
subgraph "工具库"
AXIOS[Axios HTTP客户端]
UUID[UUID生成器]
BACKOFF[指数退避算法]
end
WEB_DEPS --> REACT_QUERY
WEB_DEPS --> LUCIDE
WEB_DEPS --> DAYJS
GIN --> CLICKHOUSE
KAFKA --> SARAMA
PGX --> CONSUMER
AXIOS --> WEB_DEPS
UUID --> SDK
BACKOFF --> SDK
```

**图表来源**
- [web/package.json](file://web/package.json)
- [server/go.mod](file://server/go.mod)

**章节来源**
- [web/package.json](file://web/package.json)
- [server/go.mod](file://server/go.mod)

## 性能考虑

### 实时性优化

系统在多个层面实现了性能优化以确保实时性：

1. **前端轮询优化**：实时页面每15秒自动刷新，平衡了实时性和性能
2. **批量处理**：Consumer 服务采用批量处理策略，减少数据库写入开销
3. **索引优化**：ClickHouse 使用合适的分区和排序键优化查询性能
4. **缓存策略**：Redis 用于缓存热点数据和会话信息

### 存储优化

```mermaid
graph LR
subgraph "存储优化策略"
PARTITION[分区策略<br/>按时间分区]
SORTKEY[排序键<br/>project_id, time]
COMPRESSION[压缩策略<br/>列式存储]
TTL[生命周期管理<br/>自动清理旧数据]
end
subgraph "查询优化"
AGGREGATION[预聚合<br/>高频查询结果缓存]
INDEX[索引优化<br/>常用过滤字段建立索引]
QUERY_PLAN[查询计划<br/>优化器选择最优执行路径]
end
PARTITION --> SORTKEY
SORTKEY --> COMPRESSION
COMPRESSION --> TTL
AGGREGATION --> INDEX
INDEX --> QUERY_PLAN
```

### 监控与告警

系统集成了完整的监控体系：

- **Prometheus 指标收集**：收集各个服务的关键性能指标
- **Grafana 可视化**：提供实时监控面板
- **日志聚合**：统一的日志收集和分析
- **健康检查**：自动化的服务健康状态检测

## 故障排除指南

### 常见问题诊断

1. **实时数据延迟**
   - 检查 Kafka 消费组状态
   - 验证 Consumer 服务运行状态
   - 监控 ClickHouse 写入性能

2. **事件丢失**
   - 检查 SDK 缓存机制
   - 验证 IndexedDB 存储状态
   - 查看 DLQ 错误队列

3. **查询性能问题**
   - 分析 ClickHouse 查询计划
   - 检查索引使用情况
   - 优化查询参数和时间范围

### 调试工具

```mermaid
flowchart TD
Problem[问题出现] --> CheckLogs[检查日志]
CheckLogs --> IdentifyStage{识别问题阶段}
IdentifyStage --> |采集阶段| DebugSDK[调试SDK]
IdentifyStage --> |传输阶段| DebugKafka[调试Kafka]
IdentifyStage --> |处理阶段| DebugConsumer[调试Consumer]
IdentifyStage --> |存储阶段| DebugClickHouse[调试ClickHouse]
IdentifyStage --> |查询阶段| DebugAPI[调试API]
DebugSDK --> MonitorMetrics[监控指标]
DebugKafka --> CheckOffsets[检查偏移量]
DebugConsumer --> VerifySchema[验证模式]
DebugClickHouse --> AnalyzeQuery[分析查询]
DebugAPI --> TestEndpoints[测试接口]
MonitorMetrics --> ResolveIssue[解决问题]
CheckOffsets --> ResolveIssue
VerifySchema --> ResolveIssue
AnalyzeQuery --> ResolveIssue
TestEndpoints --> ResolveIssue
```

**章节来源**
- [server/consumer/internal/worker/worker.go:194-210](file://server/consumer/internal/worker/worker.go#L194-L210)
- [sdk/web/src/storage.ts:46-94](file://sdk/web/src/storage.ts#L46-L94)

## 结论

AeroLog 的实时分析功能通过精心设计的架构和优化策略，实现了高性能、高可用的实时数据处理能力。系统的主要优势包括：

1. **完整的实时链路**：从事件采集到实时展示的全链路优化
2. **可靠的错误处理**：多层重试和降级机制确保数据可靠性
3. **灵活的扩展性**：模块化设计支持水平扩展
4. **完善的监控体系**：全面的指标收集和可视化展示

未来可以进一步优化的方向包括：
- 引入更复杂的事件富化规则
- 实现更精细的缓存策略
- 增强异常检测和自动恢复能力
- 优化大规模数据场景下的查询性能

通过持续的优化和改进，AeroLog 将能够更好地满足企业级实时分析的需求。