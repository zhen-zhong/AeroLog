...
## 更新摘要
**所做更改**
- 更新了Android SDK文档链接引用，确保指向完整的Android SDK文档页面
- 在项目概述中增强了Android SDK功能特性的描述
- 补充了Android SDK与其他平台SDK的一致性设计说明
...
## 简介
AeroLog是一个自研的多端埋点平台，采用现代化微服务架构设计，提供统一的数据采集-传输-存储-分析流水线。项目基于Go语言实现微服务化架构，支持Android/iOS/Web三端SDK覆盖，通过Docker容器化部署，实现了从单体架构到微服务架构的根本性转变。

AeroLog的核心目标是为应用提供可靠、可观测、高性能的用户行为数据采集能力，支持高并发写入、离线持久化、幂等去重与实时分析，满足从MVP到大规模生产的演进需求。新架构采用模块化设计，每个服务独立部署和扩展，提升了系统的可维护性和可扩展性。

**章节来源**
- [README.md:1-53](file://README.md#L1-L53)

## 项目结构
项目采用微服务架构的模块化组织方式，分为SDK、服务端微服务、前端和部署四个主要部分：

```mermaid
graph TB
subgraph "SDK层"
ANDR["Android SDK<br/>Kotlin + Room<br/>完整文档链接"]
IOS["iOS SDK<br/>Swift + SQLite"]
WEB["Web SDK<br/>TypeScript + IndexedDB"]
end
subgraph "微服务层"
COL["Collector Service<br/>接收层"]
CON["Consumer Service<br/>ETL+写入"]
API["API Service<br/>管理与查询"]
PKG["公共库<br/>模型/消息队列"]
end
subgraph "数据存储"
...
- [server/go.work:1-20](file://server/go.work#L1-L20)
...
### 微服务化架构设计
- **服务拆分**：将原有单体应用拆分为独立的微服务，每个服务职责单一且可独立部署
- **统一协议**：三端SDK遵循相同的上报协议，确保数据格式一致
- **容器化部署**：基于Docker的微服务架构，支持弹性扩缩容
- **服务发现**：通过环境变量和服务注册实现服务间的通信
...
- **完整文档支持**：每个平台都提供详细的接入文档和最佳实践

### 微服务Go架构
- **Collector Service**：高并发接收层，负责鉴权、限流、Schema校验
- **Consumer Service**：Kafka消费与ETL处理，完成UA/IP解析与数据富化
- **API Service**：提供管理与查询接口，支持分析报表
- **公共库**：统一的事件模型、消息队列封装
...
## 架构总览
AeroLog采用微服务架构，参考神策的成熟实践，实现数据从采集到分析的完整闭环：

```mermaid
graph LR
subgraph "数据采集层"
A["Android SDK<br/>完整文档支持"]
I["iOS SDK<br/>Swift + SQLite"]
W["Web SDK<br/>TypeScript + IndexedDB"]
end
subgraph "微服务层"
C["Collector Service"]
K["Kafka"]
...
### 关键监控指标
- **Collector Service指标**：QPS、p99延迟、Kafka lag、内存使用
- **Consumer Service指标**：消费速率、ETL耗时、死信队列数量
- **API Service指标**：请求响应时间、数据库连接数
- **存储指标**：ClickHouse写入延迟、磁盘空间、连接数

**章节来源**
- [docs/architecture.md:43-47](file://docs/architecture.md#L43-L47)
- [docs/observability.md:1-100](file://docs/observability.md#L1-L100)

## 结论
AeroLog作为一个现代化的多端埋点平台，在架构设计上实现了从单体架构到微服务架构的根本性转变。通过采用微服务化设计、容器化部署和现代化技术栈，实现了从数据采集到分析的完整解决方案。

### 核心优势
- **微服务化设计**：将原有单体应用拆分为独立的微服务，提升系统可维护性和可扩展性
- **一致性设计**：三端SDK共享协议与最佳实践，提供完整的Android SDK文档支持
- **高扩展性**：支持从单机到大规模集群的演进
- **高可靠性**：多级冗余与监控保障数据质量
- **易维护性**：清晰的模块划分与完善的文档

### 技术特色
- **Go语言微服务**：高性能并发处理与现代化微服务架构
- **Docker容器化**：支持弹性扩缩容和统一部署
- **Kafka消息队列**：高吞吐量数据管道
- **ClickHouse OLAP**：面向分析的高性能数据库
- **Next.js前端**：现代化的管理界面

AeroLog为团队提供了稳定可靠的埋点基础设施，既适合初学者快速上手，也为有经验的开发者提供了深入定制的空间。通过合理的微服务架构设计和完善的监控体系，能够支撑业务从起步到规模化发展的各个阶段。

**更新** 增强了Android SDK文档链接的引用，确保指向完整的Android SDK文档页面，为开发者提供更便捷的接入指导。