...
- [开发脚本](file://scripts/dev.sh)
- [Web SDK 包配置](file://sdk/web/package.json)
- [Web SDK 类型配置](file://sdk/web/tsconfig.json)
</cite>
...
**章节来源**
- [架构文档](file://docs/architecture.md)
- [API 主程序:13-25](file://server/api/cmd/main.go#L13-L25)
- [收集器主程序:13-25](file://server/collector/cmd/main.go#L13-L25)
- [消费者主程序:13-25](file://server/consumer/cmd/main.go#L13-L25)
...
- [API 主程序:9-11](file://server/api/cmd/main.go#L9-L11)
- [收集器主程序:9-11](file://server/collector/cmd/main.go#L9-L11)
- [消费者主程序:9-11](file://server/consumer/cmd/main.go#L9-L11)
- [Kafka 生产者:9-10](file://server/pkg/mq/producer.go#L9-L10)
- [Next.js 应用包配置:11-40](file://web/package.json#L11-L40)

**章节来源**
- [go.work:1-9](file://server/go.work#L1-9)
- [Next.js 应用包配置:1-42](file://web/package.json#L1-L42)
...
**章节来源**
- [API 主程序:18-25](file://server/api/cmd/main.go#L18-L25)
- [收集器主程序:18-25](file://server/collector/cmd/main.go#L18-L25)
- [消费者主程序:18-25](file://server/consumer/cmd/main.go#L18-L25)
- [docker-compose.yml:113-147](file://deploy/docker-compose.yml#L113-L147)
...
**章节来源**
- [Next.js 应用包配置:24-40](file://web/package.json#L24-L40)
- [架构文档:43-47](file://docs/architecture.md#L43-L47)

### 新增开发工具链与工作流程

#### Makefile 工具链
项目引入了完整的 Makefile 工具链，提供一键式开发体验：
...
#### 开发脚本自动化
新增的 `scripts/dev.sh` 提供智能的开发环境启动脚本，具备以下特性：
- 自动清理功能：优雅处理进程终止与资源清理
- 进程管理：统一管理多个服务进程，记录 PID 与日志
- 健康检查：自动检测服务就绪状态
- 日志聚合：集中管理服务日志输出

**章节来源**
- [开发脚本:1-79](file://scripts/dev.sh#L1-L79)

#### Web SDK 构建优化
Web SDK 采用现代化构建工具链：
- **打包工具**：tsup 替代传统 webpack，提供更快的构建速度
- **类型定义**：完整的 TypeScript 类型支持
- **测试框架**：vitest 提供快速的单元测试环境
- **开发体验**：支持热重载与实时编译

**章节来源**
- [Web SDK 包配置:1-29](file://sdk/web/package.json#L1-L29)
- [Web SDK 类型配置:1-17](file://sdk/web/tsconfig.json#L1-L17)

#### 基础设施配置更新
Docker Compose 配置已更新为 Redpanda 替代传统 Kafka：
- **Redpanda 优势**：无需 Zookeeper，单机友好，支持平滑迁移
- **端口映射**：Kafka API 映射到 19092 端口，避免与业务 API 冲突
- **管理界面**：集成 Redpanda Console 提供可视化管理
- **版本升级**：使用最新稳定版本确保安全性与性能

**章节来源**
- [docker-compose.yml:37-73](file://deploy/docker-compose.yml#L37-L73)

#### 环境变量标准化
各服务的环境变量配置已标准化，支持灵活的部署场景：
- **API 服务**：AEROLOG_API_ADDR、AEROLOG_METRICS_ADDR、JWT 配置
- **收集器**：AEROLOG_ADDR、AEROLOG_KAFKA_BROKERS、Redis 连接
- **消费者**：Kafka 消费组配置、批量处理参数
- **通用配置**：Postgres DSN、ClickHouse 连接信息

**章节来源**
- [API 配置:24-37](file://server/api/internal/config/config.go#L24-L37)
- [收集器配置:20-29](file://server/collector/internal/config/config.go#L20-L29)
- [消费者配置:28-44](file://server/consumer/internal/config/config.go#L28-L44)